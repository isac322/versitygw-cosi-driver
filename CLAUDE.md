...
- **Chart**: bump `version` in `deploy/helm/versitygw-cosi-driver/Chart.yaml` and push to master → auto-creates `chart-v<version>` tag and releases to GHCR OCI registry (`release-chart.yaml`)
- When bumping chart version, **always** update the `artifacthub.io/changes` annotation in `Chart.yaml` with the changes for that version. Artifact Hub reads this annotation, not `CHANGELOG.md`
